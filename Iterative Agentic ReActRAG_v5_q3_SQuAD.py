...
SYSTEM_PROMPT = """You are an expert system designed to answer questions based on squad-style (JSONL, similar to NQ) datasets.
...
def convert_squad_sample_to_text(sample, include_context=True, max_context_len=5000):
    """Convert a single SQuAD JSON sample into a readable text format including all keys."""
...
for i, sample in tqdm(enumerate(data), total=len(data)):
    try:
        ...
        extracted_text, question = convert_squad_sample_to_text(sample)
...
df.to_csv("ReActRAG_v5_Qwen3-8B_SQuAD.csv", index=False, encoding="utf-8")

print(f"Wrote ReActRAG_v5_Qwen3-8B_SQuAD.csv with {len(results)} rows.")

# json output

with open("ReActRAG_v5_Qwen3-8B_SQuAD.json", "w", encoding="utf-8") as f:
    json.dump(results, f, ensure_ascii=False, indent=2)

print(f"Wrote ReActRAG_v5_Qwen3-8B_SQuAD.json with {len(results)} rows.")
